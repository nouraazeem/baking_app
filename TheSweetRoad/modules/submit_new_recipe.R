  ...
  #################################################### HOT 1 ###############################################              
                 output$submit_new_name_hot <- renderRHandsontable({
                   ...
                   submitter_details = data.frame(recipe_submitter = recipe_submitter,
                                                  ...
                                                  servings_total = servings_total, stringsAsFactors = FALSE)
                   # DROPDOWN OPTIONS -- sweetness + cake type
                   sweetness <- 1:5
                   types_of_desserts <- c("Cake", "Cake Pop", "Cheesecake", "Cookie", "Pie/Tart",
                   ...
                   rhandsontable(submitter_details,
                                 rowHeaders = NULL,
                                 width = 1000,
                                 height = 250, colHeaders = c("Recipe Submitter", "Recipe Name",
                     ...
                     hot_col(col = 1, colWidths = 150, valign = 'htCenter') %>% 
                     # Specify column width + center the text within the HOT
                     hot_col(col = 2, colWidths = 150, valign = 'htCenter') %>% 
                     # Specify column width + center the text within the HOT + allow users to select from the 
                     # types_of_desserts dropdown
                     hot_col(col = 3, type = "dropdown", source = types_of_desserts, colWidths = 150,
                             valign = 'htCenter') %>% 
                     # Specify column width + center the text within the HOT + allow users to select from the 
                     # sweetness scale dropdown
                     hot_col(col = 4, type = "dropdown", source = sweetness, colWidths = 150, 
                             valign = 'htCenter') %>% 
                     hot_col(col = 5, colWidths = 150, valign = 'htCenter') %>% 
                     # Specify row heights
                     hot_rows(rowHeights = 35)
                 })
                 
                 # I can make some changes to this rhandsontable and generate the resulting table as dataframe
                 values_submitter <- reactiveValues(submitter_data = NULL)
                 
                 observe({
                   values_submitter$submitter_data <- hot_to_r(input$submit_new_name_hot)
                 })
                 ...
                 submitter_info <- submitter_df() %>% 
                   select(recipe_submitter, recipe_name)
...
#################################################### HOT 2 ###############################################              
                 ...
                 output$submit_new_ing_hot <- renderRHandsontable({
                   ...
                   Ingredient = c("Sugar", "Flour", "Almonds", "Vanilla Extract", "Butter",
                                  "Cinnamon")
                   # Create a dataframe with the two columns
                   ingredients_df = data.frame(Amount = Amount,
                                               Ingredient = Ingredient, stringsAsFactors = FALSE)
                   
                   rhandsontable(ingredients_df,
                                 rowHeaders = NULL,
                                 width = 800,
                                 height = 300, colHeaders = c("Enter the amount of the ingredient",
...
#################################################### HOT 3 ###############################################              
                 ...
                 Step.number = 1:5
                 # Recipe Steps
                 recipe_step_details = c("Mix all the dry ingredients", "Mix all the wet ingredients",
                 ...
                 output$submit_new_steps_hot <- renderRHandsontable({
                   ...
                   step_details = data.frame(Step.number = Step.number,
                                             recipe_step_details = recipe_step_details, 
                                             stringsAsFactors = FALSE)
                   
                   rhandsontable(step_details,
                                 rowHeaders = NULL,
                                 width = 500,
                                 height = 500, colHeaders = c("Step Number", "Step Details")) %>%
                     hot_context_menu(allowRowEdit = TRUE) %>%
